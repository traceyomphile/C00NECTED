...
class CallManager:
    ...
    def create_udp_socket(self) -> int:
        ...
        port = self.udp_sock.getsockname()[1]

        # Enable reuse so we can rebind quickly afterr call ends
            ...
            try:
                ...
                if self.call_ended:
                    if pkt_type in (PKT_AUDIO, PKT_VIDEO):
                        ...
                        self.gui_queue(("INCOMING_CALL_UDP",))
                else:
                    if pkt_type == PKT_AUDIO:
                        self._audio_queue.put(datagram[1:])
                    elif pkt_type == PKT_VIDEO:
                        self._video_queue.put(datagram[5:])
                    elif pkt_type == PKT_END:
                        self.call_ended = True
                        self.gui_queue(("CALL_ENDED_REMOTE",))
    ...
    def end_call(self):
        ...
        if self.udp_sock and self.peer_addr:
            try: self.udp_sock.sendto(PKT_END, self.peer_addr)
            except: pass
        self.peer_addr              = None
    ...
    def listen_for_incoming(self):
        ...
        while True:
            # Wait if a call is in progress — don't touch the socket
    ...
    def _recv_video(self):
        while not self.call_ended:
            ...
            if pkt == PKT_END:
                ...
                self._call_done_event.set()
                break
            if pkt != PKT_VIDEO or len(datagram) < 5:
                continue
...
class NetworkClient:
    ...
    def send_file(self, recipient: str, filepath: str):
        ...
        send_framed_msg(self.tcp_sock, f"GET_PEER:{recipient}:{fname}", 'C')
    ...
    def disconnect(self):
        ...
        try: send_framed_msg(self.tcp_sock, "EXIT:", 'C')
        except: pass
        ...
        try: self.call_manager.udp_sock.close()
        except: pass
        ...
        while True:
            try:
                ...
                if msg_type == 'C' and msg.startswith("PEER_INFO:"):
                    parts = msg.split(':')
                    target_user, t_ip, t_port = parts[1], parts[2], parts[3]
                ...
                if msg_type == 'C' and msg.startswith("GROUP_PEER_INFO:"):
                    parts = msg.split(':', 2)
                    group_name, peers_str = parts[1], parts[2]
                ...
                if msg_type == 'D' and msg.startswith("FILE:"):
                    ...
                    if len(parts) >= 4:
                        ...
                        self._save_b64_file(fname, ftype, b64, sender)
                    continue
    ...
    def _tcp_send_file(self, filepath: str, target_ip: str, target_port: int, recipient: str):
        try:
            ...
            sock.sendall(f"FILE:{fname}:{filesize}:{self.username}\n".encode('utf-8'))
            with open(filepath, 'rb') as f:
                while chunk := f.read(4096):
                    sock.sendall(chunk)
            sock.close()
            self.gui_queue.put(('FILE_SENT', fname, recipient))
        except Exception as e:
    ...
    def _upload_offline(self, recipient: str, filepath: str):
        try:
            ...
            send_framed_msg(self.tcp_sock, f"UPLOAD_MEDIA:{recipient}:{fname}|{ftype}|{b64}", 'D')
            self.gui_queue.put(('STATUS', f'📤 "{fname}" queued for {recipient} (offline)'))
        except Exception as e:
    ...
    def _handle_file_conn(self, conn: socket.socket, addr):
        try:
            ...
            parts  = header.split(":")
            if len(parts) < 3 or parts[0] != 'FILE': return
            fname    = parts[1]
            filesize = int(parts[2])
            # Sender is optional 4th field (added in updated client)
            sender   = parts[3] if len(parts) >= 4 else None

            save_path = self._unique_path(os.path.join("received", fname))
            received  = 0
            ...
            self.gui_queue.put(('FILE_RECEIVED', fname, get_file_type(fname), save_path, sender))
        except Exception:
            pass
        finally:
            conn.close()

    def _save_b64_file(self, fname: str, ftype: str, b64: str, sender: str = None):
        try:
            os.makedirs("received", exist_ok=True)
            save_path = self._unique_path(os.path.join("received", fname))
            with open(save_path, 'wb') as f:
                f.write(base64.b64decode(b64))
            self.gui_queue.put(('FILE_RECEIVED', fname, ftype, save_path, sender))
        except Exception:
            pass
...
class AuthWindow:
    ...
    def _get_field(self, placeholder: str) -> str:
        ...
        var, entry = self._entries.get(placeholder, (None, None))
...
class ChatWindow:
    ...
    def _build_layout(self):
        ...
        # ── Sidebar ──
        self.sidebar = tk.Frame(self.root, bg=C_SIDEBAR, width=320)
    ...
    def _build_sidebar(self):
        ...
        self.chat_listbox = tk.Listbox(
            list_frame, bg=C_SIDEBAR, fg=C_TEXT, font=FONT_APP,
        ...
        self.chat_listbox.bind("<Double-Button-1>", self._on_chat_select)
        ...
        for chat_id in sorted(self.conversations.keys()):
            ...
            for m in reversed(msgs):
                ...
                if c and m.get('type') != 'system':
                    last = c[:34] + '…' if len(c) > 34 else c
                    break
            prefix  = '👥 ' if self._is_group_chat(chat_id) else '💬 '
            unread  = self.unread_counts.get(chat_id, 0)
            badge   = f'  [{unread}]' if unread else ''
            self.chat_listbox.insert('end', f"  {prefix}{chat_id}{badge}\n  {last}")
    ...
    def _on_chat_select(self, _event=None):
        ...
        if not sel: return
        raw  = self.chat_listbox.get(sel[0])
        line = raw.strip().split('\n')[0].strip()
        for prefix in ('👥 ', '💬 '):
            line = line.replace(prefix, '')
        # Strip unread badge like "[3]"
    ...
    def _handle_event(self, event: tuple):
        ...
        # UDP listener detected incoming audio/video before TCP signal arrived
            ...
            _, fname, recipient = event
            self._show_status(f"✅ '{fname}' sent to {recipient}")

        elif etype == 'FILE_RECEIVED':
            # Unpack — sender is optional (older protocol omits it)
            _, fname, ftype, save_path = event[:4]
            ...
            # ── Voice note ────────────────────────────────────────────────────
            if fname.startswith('c00n_voice_') and fname.endswith('.wav'):
            ...
            # ── Regular file ──────────────────────────────────────────────────
            self._show_status(f"📥 Received: {fname}")
            note = {
                'type':    'system',
                'content': f"📎 {fname}  →  {save_path}",
            }
