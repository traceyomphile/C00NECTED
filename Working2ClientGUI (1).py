...
class ChatWindow:
    ...
    def _handle_event(self, event: tuple):
        ...
        if etype == 'MESSAGE':
            ...
            self._append_message(recipient, note)
            ...
            file_ftype = get_file_type(filename)
            if file_ftype == 'image' and PIL_AVAILABLE and target_chat:
                try:
                    ...
                    bubble = tk.Frame(outer, bg=C_RECV, padx=8, pady=6)
                    bubble.pack(anchor='w', padx=4)

                    if sender:
                        tk.Label(bubble, text=sender, font=("Segoe UI", 8, "bold"),
                                 fg=C_ACCENT, bg=C_RECV).pack(anchor='w')

                    img_lbl = tk.Label(bubble, image=photo, bg=C_RECV, cursor='hand2')
                    img_lbl.image = photo
                    img_lbl.pack(anchor='w')
                    img_lbl.bind('<Button-1>', lambda e, p=save_path: os.startfile(p)
                                 if os.name == 'nt' else os.system(f'xdg-open "{p}" &'))

                    tk.Label(bubble, text=filename, font=FONT_MICRO,
                             fg=C_SECONDARY, bg=C_RECV).pack(anchor='w', pady=(2, 0))
                    tk.Label(bubble, text=ts.split(' ')[1], font=FONT_MICRO,
                             fg=C_SECONDARY, bg=C_RECV).pack(anchor='e')

                    self.msg_canvas.update_idletasks()
                    self._scroll_to_bottom()

                    if target_chat not in self.conversations:
                        self.conversations[target_chat] = []
                    self.history.append(target_chat, {
                        'type': 'system',
